...
export class BankAccount {
  ...
  onSubmit() {
    ...
    const accountData: any = {
      accountNo: this.accountForm.get('accountNo')?.value ?? '',
      ...
      totalAmount: this.accountForm.get('totalAmount')?.value ?? '',
    };
    ...
    let serviceCall = this.bankAccountService.createAccount(formData);
    if (this.editingId) {
      accountData.id = String(this.editingId);
      formData.append('removedFiles', JSON.stringify(this.removedFiles));
      serviceCall = this.bankAccountService.updateAccount(formData);
    }
    formData.append('bankAccountData', JSON.stringify(accountData));
    serviceCall
      .pipe(
        finalize(() => {
          this.saving = false;
  ...
  openFile(id: any, event?: Event) {
    ...
    this.bankAccountService.getFileContent(id).subscribe({
      next: (blob) => {
        console.log(blob);
