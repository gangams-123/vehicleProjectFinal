...
export const createBranchWithAddressAndFile = async (req: Request, res: Response) => {
 ...
 if (typeof addresses === "string") {
  try {
    ...
    return res.status(400).json({ message: "Invalid JSON format for addresses" });
  }
}
...
export const getBranchAddress = async (req: Request, res: Response) => {
  try {
    ...
    // Fetch addresses
    const addresses = await Address.findAll({ where: { entityId: branchId, entityType: 'branch' } });
...
export const getBranchFiles = async (req: Request, res: Response) => {
  try {
    ...
    // Fetch addresses
    const addresses = await File.findAll({ where: { entityId: branchId, entityType: 'branch' } });
