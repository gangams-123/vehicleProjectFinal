...
export const createWorkFlowWithChild = async (req: Request, res: Response) => {
  ...
  try {
    ...
    // 1️⃣ Create Vendor
    const newWorkFlow = await WorkFlowMain.create(workFlowData, {
      transaction: t,
    });
    ...
    if (workFlows && workFlows.length > 0) {
      ...
      newWorkFlowChild = await WorkFlowChild.bulkCreate(workFlowChildData, {
        transaction: t,
      });
    }
    ...
    res
      .status(500)
      .json({ message: "Failed to create workflow with child", error });
  }
};
...
export const checkWorkFlowActiveExists = async (
  req: Request,
  res: Response
) => {
  try {
    ...
    const existingWorkflow = await WorkFlowMain.findOne({
      where: { module: moduleName, status: status },
    });
    if (existingWorkflow) {
    ...
    return res
      .status(404)
      .json({ message: "some error occured while finding not found" });
  }
};
...
export const changeStatus = async (req: Request, res: Response) => {
  try {
    ...
    if (affectedCount === 0)
      return res
        .status(404)
        .json({ message: "some problem did not change the status" });
    else res.json({ message: "changed successfully" });
  } catch (error) {
    res.status(500).json({ error: "Internal Server Error" });
  }
};

export const getWorkflowByStatusByModule = async (
  req: Request,
  res: Response
) => {
  try {
      ...
      include: [
        {
          model: WorkFlowChild,
          as: "WorkFlowChild",
          attributes: ["status", "roleId", "stepOrder"],
        },
      ],
    });

    if (existingWorkflow) {
      const workflowData = existingWorkflow.toJSON();
      const groupedData: {
        [status: string]: {
          roleId: string;
          workflowId: number;
      ...
      workflowData.WorkFlowChild?.forEach((child) => {
        if (!groupedData[child.status]) {
          groupedData[child.status] = [];
        }
        groupedData[child.status].push({
          roleId: child.roleId.toString(),
